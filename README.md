...
dirt roads, wrap routes around residence plots, snap them into a connected
network, and generate timber bridge structures where roads cross water.

A residence frontage tool lays out adjustable roadside plots and terrain-aligned
cottages. Narrow capped access lanes connect each home back to the public road,
and a dedicated tool builds low dry-stone walls along road shoulders.
...
- Exterior residence-boundary snapping with rounded plot-corner paths and aligned handoffs into existing roads
...
- Road-clear residence connection circles plus slim access lanes with rounded, feathered caps at house entrances
- Crisp dirt-to-timber bridge thresholds, deck-scaled timber UVs, rounded approach hubs, corrected supports, and terrain-following railings
- Deterministic two-course dry-stone walls with chipped-stone variants, dedicated limestone PBR textures, road-shoulder snapping, terrain foundations, and road-overlap rejection
...
- `L` toggles the roadside dry-stone wall tool
...
- While placing walls: start on a road shoulder, click along the verge, then press `Enter`; `Alt` + click a wall to remove it
...
Create a production build with `npm run build`. Run the road/bridge and stone-wall
regression suites with `npm test`.
